...
function BookingList() {
  ...
  const getBookings = async () => {
    try {
      const response = await axios.get('http://localhost:8800/api/bookings/', { withCredentials: true });
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (confirmDelete) {
        await axios.delete(`http://localhost:8800/api/bookings/checkout/${id}`); // Adjust the API route accordingly
        getBookings();
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="booking-list-container">
      <div className="header">
        ...
        <Link to="/portal/create-bookings" className="create-button">
